...
function love.update(dt)
    ...
    if input:pressed('damage') then
        local damage = 50
        timer:tween(0.25, health_top, {
            w = health_top.w - damage,
            x = health_top.x - damage / 2
        }, 'in-cubic', function()
            timer:tween(2, health_bottom, {
                w = health_bottom.w - damage,
                x = health_bottom.x - damage / 2
            }, 'in-cubic')
        end)
    end
...
function love.draw()
    ...
    love.graphics.print("mouseX: " .. mouseP[001] .. " mouseY: " .. mouseP[002] .. " mouseButton: " .. mouseP[003], 0,
        60)
    ...
    love.graphics.rectangle('fill', health_bottom.x - health_bottom.w / 2, health_bottom.y - health_bottom.h / 2,
        health_bottom.w, health_bottom.h)

    love.graphics.setColor(health_top.color.red, health_top.color.green, health_top.color.blue)
    love.graphics.rectangle('fill', health_top.x - health_top.w / 2, health_top.y - health_top.h / 2, health_top.w,
        health_top.h)
end
...
function start_cross_animation()
    timer:tween(1, rect_1, {
        w = 0
    }, 'in-out-cubic', function()
        timer:tween(1, rect_2, {
            h = 0
        }, 'in-out-cubic', function()
            timer:tween(1, rect_1, {
                w = 50
